...
Y1_right  =   Y_right
X1_right  =   X_right                                                                           
L1_m = L1-Y1_right- np.abs(X1_right)+(yc_pad-Y_right) #calculate the arc1 length by subtracting the lengh of 
                                       #connection
...
if Width_T<W_pad_x: 
    if Width_T<W_pad_y:         
        ...
        Y_right =  yc_pad-W_pad_y/2+Width_T/2# align the y coordinate on the right side of arc1 with the end of the pad    
    L_m = L + 2 *(yc_pad- Y_right) - (X_left-XL_R) - (-Width_T / 2 + W_pad_x / 2)
    R_m = L_m/theta_trace
    ...
    if d < W_pad_y/2:
        ...
        Y_right =  yc_pad-W_pad_y/2+Width_T/2 #align the end of the arc on the bottom of the pad
    L_m = L  + 2 *(yc_pad- Y_right)  - (X_left-XL_R)-1/3*yc_pad
    R_m = L_m/theta_trace
    X_right= X_left + 2*R_m*np.sin(1/2*theta_trace)
    XR_R = X_right
...
if Width_T<W_pad_x: 
    ...
    if d < W_pad_y:
        ...
        Y_right =  yc_pad-W_pad_y/2+Width_T/2 #align the end of the arc on the bottom of the pad
    L_m = L + 2 *(yc_pad- Y_right) - (X_left-XL_R)
    R_m = L_m/theta_trace
    X_right= X_left + 2*R_m*np.sin(1/2*theta_trace)
    XR_R = X_right
